...
9. **Release v1.7.74 (Build 119):** Publicada en GitHub Releases vía CI/CD con tag `v1.7.74` (versionCode 119). Implementa vinculación y anclaje por red Wi-Fi (`wifiSsid`) en Zonas Seguras de Thor Radar (100% de certeza en interiores y 0 falsas alertas), compuerta de bloqueo estricto en reposo (`STILL` / velocidad < 2 km/h) para eliminar falsas salidas por jitter de GPS cuando el usuario está quieto o durmiendo, filtro de precisión GPS para salidas ($\le 45\text{m}$), ventana de permanencia temporal continua (Dwell-Time de 90s a 120s) y badges de estado Wi-Fi en la lista de zonas.
...
1. **Arquitectura y Servicios de Rastreo (`ThorRadarManager.kt` & `ThorRadarService.kt`):**
   ...
   - **Emisión de Latidos (Heartbeats):** Registro automático de cambios de estado, velocidad, actividad y geocodificación inversa para dirección física (`thoroughfare`, `locality`).
    - **Notificaciones Push Automáticas de Zonas Seguras, Anclaje Wi-Fi y Filtro Anti-Spam / Anti-Jitter (FCM v1 Data-Only):**
      - **Vinculación y Anclaje por Red Wi-Fi (`wifiSsid`):** Permite asociar la red Wi-Fi del hogar o lugar de estudio/trabajo a la zona segura. Mientras el dispositivo permanezca conectado a ese Wi-Fi, la app tiene **100% de certeza** de ubicación interior y **bloquea por completo cualquier falsa salida** provocada por fluctuaciones o saltos del GPS. Además, conectarse al Wi-Fi confirma la llegada a la zona en 0 segundos.
      - **Bloqueo Estricto por Estado en Reposo (`STILL` / Velocidad < 2.0 km/h):** Si el dispositivo está estático (en la cama, sofá o cargando), el algoritmo ignora cualquier salto de coordenadas por degradación de señal en interiores, impidiendo terminantemente disparar alertas de salida sin movimiento físico real.
      - **Filtro de Precisión para Salidas ($\le 45\text{m}$) y Compuerta Temporal (Dwell-Time):** Las salidas de zona solo se evalúan si el GPS adquiere una precisión limpia al aire libre ($\le 45\text{m}$) y el usuario permanece continuamente fuera de la zona por al menos **90 a 120 segundos** ($\ge 3-4$ muestras consecutivas alejándose), o si se aleja conduciendo en vehículo.
      ...
      - **Ventana de Cooldown y Anti-Flapping:** Ventana de seguridad para evitar re-notificaciones en bucle cuando el usuario se desplaza por el perímetro limítrofe de la zona.
