...
- [ProfileSettingsCompose.kt](file:///home/kevin/Escritorio/Proyectos/Diario_alikevin/app/src/main/java/calendario/kevshupp/diariokevinali/compose/ProfileSettingsCompose.kt): Pantalla de Configuración principal (menú de navegación a Perfil, Sincronización, Ajustes Avanzados, Panel Admin y Limpiador de Duplicados).
...
- [AdminSettingsCompose.kt](file:///home/kevin/Escritorio/Proyectos/Diario_alikevin/app/src/main/java/calendario/kevshupp/diariokevinali/compose/AdminSettingsCompose.kt): Panel de administración protegido por contraseña para reinicio de estadísticas de minijuegos, ranking de cuidadores y mascotas.
- [AdvancedSettingsCompose.kt](file:///home/kevin/Escritorio/Proyectos/Diario_alikevin/app/src/main/java/calendario/kevshupp/diariokevinali/compose/AdvancedSettingsCompose.kt): Panel de ajustes avanzados y estado de conexiones (Firebase, Cloudinary, Drive).
...
5. **Módulos Optimizados (4 Pestañas Claras):**
   ...
   - **Asistente de Configuración Requisitos (`RadarSetupWizardDialog`):** Diálogo retro interactivo con checklist paso a paso. Se persiste el flag de descarte (`radar_setup_wizard_dismissed`) para no interrumpir al usuario en aperturas posteriores y permanece accesible desde la pestaña Ajustes.
    - **Magic Packet Wake-on-LAN de Ubicación en Vivo (`[ 🔄 ACTUALIZAR ]` / `[ 🔄 ACTUALIZAR AHORA ]` - v1.7.50):** Envía un paquete prioritario FCM (`radar_ping` / `WOL_LOCATION_WAKEUP`) con `priority: HIGH` y TTL. Al recibirlo, `MyFirebaseMessagingService` invoca `ThorRadarManager.handleMagicLocationPing(context)` de forma silenciosa, el cual adquiere un `WakeLock` seguro (45s), emite inmediatamente un Heartbeat con batería y última posición a Firestore (<300ms), reactiva `ThorRadarService` y dispara un Fix GPS de alta precisión multicanal (`FusedLocation` + `LocationManager` nativo) que actualiza las coordenadas en tiempo real incluso con el celular bloqueado o en Doze mode.
