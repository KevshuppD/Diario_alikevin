...
public class SettingsFragment extends Fragment {
    ...
    private void setupColorButtons(View v) {
        TextView tvLabelColor = (TextView) v.findViewById(R.id.tvLabelColor);
        LinearLayout colorLayout = (LinearLayout) v.findViewById(R.id.colorSelectionLayout);

        if ("Pixel Oscuro".equals(theme)) {
            tvLabelColor.setText("COLOR DE BARRAS (TEMA OSCURO)");
            tvLabelColor.setVisibility(View.VISIBLE);
            colorLayout.setVisibility(View.VISIBLE);

            // Colores para modo oscuro
            v.findViewById(R.id.colorPurple).setOnClickListener(v1 -> saveDarkColor("#7C3AED"));
            v.findViewById(R.id.colorBlue).setOnClickListener(v1 -> saveDarkColor("#0EA5E9"));
            v.findViewById(R.id.colorGreen).setOnClickListener(v1 -> saveDarkColor("#10B981"));
            v.findViewById(R.id.colorPink).setOnClickListener(v1 -> saveDarkColor("#EC4899"));
            v.findViewById(R.id.colorOrange).setOnClickListener(v1 -> saveDarkColor("#F97316"));
            v.findViewById(R.id.colorCyan).setOnClickListener(v1 -> saveDarkColor("#06B6D4"));
            v.findViewById(R.id.colorBrown).setOnClickListener(v1 -> saveDarkColor("#92400E"));
        } else {
            tvLabelColor.setText("COLOR DE BARRAS (TEMA CLARO)");
            tvLabelColor.setVisibility(View.VISIBLE);
            colorLayout.setVisibility(View.VISIBLE);
    ...
    private void saveDarkColor(String hex) {
        if (getActivity() instanceof MainActivity) {
            // Guardar PRIMERO en SharedPreferences
            requireActivity().getSharedPreferences("DiarioPrefs", Context.MODE_PRIVATE).edit().putString("darkColor", hex).apply();
            // LUEGO aplicar el tema con el color nuevo
            MainActivity main = (MainActivity) getActivity();
            main.applyTheme("Pixel Oscuro", null, hex);
            Toast.makeText(getContext(), "Color oscuro aplicado", Toast.LENGTH_SHORT).show();
        }
    }
}
